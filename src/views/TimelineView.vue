      ...
      <div
        ...
        <div v-if="evolutionData" class="mb-6 shrink-0">
          <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-2">
              <label class="text-xs font-bold">KHOẢNG THỜI GIAN:</label>
              <select
              ...
              <div class="flex flex-col gap-2 max-h-[200px] overflow-y-auto">
                <div
                  ...
                  <div class="text-right">
                    <div class="text-neo-green font-semibold">
                      +{{ author.linesAdded }}
                    </div>
                    <div class="text-neo-red font-semibold">
                      -{{ author.linesDeleted }}
                    </div>
                  </div>
                </div>
              ...
              <div
                ...
                <svg
                    ...
                    :points="
                      timelineChart
                        .map(
                          (point, i) =>
                            `${i * 80 + 40},${
                              210 - (point.commits / maxCommitsInDay) * 190
                            }`
                        )
                        .join(' ')
                    "
                    ...
                    :points="
                      timelineChart
                        .map(
                          (point, i) =>
                            `${i * 80 + 40},${
                              210 - (point.linesAdded / maxLinesInDay) * 190
                            }`
                        )
                        .join(' ')
                    "
                    ...
                    :points="
                      timelineChart
                        .map(
                          (point, i) =>
                            `${i * 80 + 40},${
                              210 - (point.linesDeleted / maxLinesInDay) * 190
                            }`
                        )
                        .join(' ')
                    "
                  ...
                  <g
                    ...
                    <circle
                      :cx="index * 80 + 40"
                      :cy="210 - (point.commits / maxCommitsInDay) * 190"
                      r="5"
                      fill="#3b82f6"
                      stroke="#000"
                      stroke-width="2" />
                    ...
                    <circle
                      :cx="index * 80 + 40"
                      :cy="210 - (point.linesAdded / maxLinesInDay) * 190"
                      r="5"
                      fill="#22c55e"
                      stroke="#000"
                      stroke-width="2" />
                    ...
                    <circle
                      :cx="index * 80 + 40"
                      :cy="210 - (point.linesDeleted / maxLinesInDay) * 190"
                      r="5"
                      fill="#ec4899"
                      stroke="#000"
                      stroke-width="2" />
                ...
                <!-- X-axis Labels - same width container -->
                <div class="flex">
                  <div
                    v-for="(point, index) in timelineChart"
                    :key="`label-${index}`"
                    ...
                    <div
                      ...
                      <div class="flex items-center gap-2">
                        <span class="text-neo-blue">●</span>
                        <span>{{ point.commits }} commits</span>
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="text-neo-green">●</span>
                        <span
                          >+{{ point.linesAdded.toLocaleString() }} dòng</span
                        >
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="text-neo-pink">●</span>
                        <span
                          >-{{ point.linesDeleted.toLocaleString() }} dòng</span
                        >
                      </div>
                      <div
        ...
        <!-- Load Evolution Button -->
        <div v-if="!evolutionData && !loadingEvolution" class="mb-6">
          <button
            @click="loadEvolution"
...
// Evolution data computed properties
const evolutionData = computed(() => repositoryStore.evolutionData);

const topAuthors = computed(() => {
  if (!evolutionData.value) return [];
  return evolutionData.value.authors.slice(0, 5);
});

const topFileTypes = computed(() => {
  if (!evolutionData.value) return [];
  return evolutionData.value.fileTypes.slice(0, 5);
});

const totalAdditions = computed(() => {
  if (!evolutionData.value) return 0;
  return evolutionData.value.codeChurn.reduce((sum, c) => sum + c.additions, 0);
});

const totalDeletions = computed(() => {
  if (!evolutionData.value) return 0;
  return evolutionData.value.codeChurn.reduce((sum, c) => sum + c.deletions, 0);
});

const netChange = computed(() => totalAdditions.value - totalDeletions.value);
...
const maxCommitsInDay = computed(() => {
  if (!timelineChart.value.length) return 1;
  return Math.max(...timelineChart.value.map((p) => p.commits), 1);
});

const maxLinesInDay = computed(() => {
  if (!timelineChart.value.length) return 1;
  return Math.max(...timelineChart.value.map((p) => p.linesAdded), 1);
});
...
async function loadEvolution() {
  ...
  try {
    await repositoryStore.loadEvolutionData(evolutionMonths.value);
    console.log("Evolution data loaded:", repositoryStore.evolutionData);
    console.log("Timeline chart:", timelineChart.value);
    console.log("Max commits:", maxCommitsInDay.value);
    console.log("Max lines:", maxLinesInDay.value);
  } catch (e) {
